...
Self-hosted instances send anonymous install-level events (version, Node/OS info, a random instance UUID) so we can see how many people run Laminar and on which versions. No trace, span, workspace, project, user, or API-key data is collected. See [Telemetry](/self-hosting/telemetry) for the full payload and how to opt out.
