...
Docker Compose runs the full Laminar stack on a single machine: frontend, app server, query engine, PostgreSQL, ClickHouse, Redis, RabbitMQ, and Quickwit. It is the fastest way to self-host for local development and small single-node deployments. For production on Kubernetes, use the [Helm chart](/self-hosting/kubernetes) instead.
