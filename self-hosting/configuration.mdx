...
The three Laminar containers (frontend, app server, query engine) are pulled from public `ghcr.io/lmnr-ai/*-ee` images, so most installs never touch this section. For production, pin a specific tag instead of `latest` so a pod restart does not pick up a newer build. The three are released together: keep their tags in sync.
