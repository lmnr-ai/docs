...
Chat-with-trace and SQL-with-AI run through one unified LLM provider configuration. Set the provider on all three application pods (they default to `gemini`), and supply the key:
...
<Note>
[Signals](/signals/introduction) and the [playground](/playground) do not use these values. Each Signal runs on an [LLM profile](/platform/llm-profiles): a workspace-level entry holding a provider, its credentials, and the models you allow. Set `LLM_API_KEY` for the features above, and add profiles in workspace settings for Signals and the playground.
</Note>
...
[Signals](/signals/introduction) extract structured events from your traces. On a self-hosted deployment, each Signal runs on an [LLM profile](/platform/llm-profiles) that a workspace member creates in the app: the provider, its credentials, and the models the Signal may use live in the profile, not in `laminar.yaml`. Signals are **enabled by default** in the frontend, and no `LLM_API_KEY` is required to turn them on.

Signals created before profiles existed keep running on the `LLM_PROVIDER` configuration above until you point them at a profile. Signals that use a profile ignore `LLM_PROVIDER`, `LLM_BASE_URL`, and the `LLM_MODEL_*` overrides entirely.
...
<Note>
Signal extraction runs in the `app-server-consumer` pod. For a Signal without a profile, that pod's `llmProvider` (and any model overrides) governs how the Signal is computed, so keep the provider consistent across `frontend`, `appServer`, and `appServerConsumer` unless you have a reason to split them. Profile credentials are encrypted by the frontend and decrypted by the app server, both with `AEAD_SECRET_KEY`, so that value has to match across pods (the chart sets one key for all of them).
</Note>
...
The [PII redactor](/platform/pii-redaction) is an optional CPU-only gRPC service that detects personally identifiable information in spans at ingest. It backs both redaction modes: **On** replaces PII before the span is persisted, and **Dual** stores the original text once and redacts it per reader, so owners and admins see the original while members, API keys, and CLI sessions see placeholders. The service is **disabled by default** and has no database or secret dependencies (the detection model is baked into the image). When enabled, the chart injects `PII_REDACTOR_URL` into the `app-server` and `app-server-consumer` pods, activating detection during span processing; the mode is then chosen per project under **Project Settings → Security**.
...
PII detection is CPU-intensive ONNX inference and the model weighs about 5.5 GB in memory, so pin it to a dedicated compute-optimized node and size it generously. For more throughput, add replicas rather than a bigger node:
