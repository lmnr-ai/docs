...
We will use to main methods here – `Laminar.start_span()` and `use_span`. You can import 
...
We are working hard on adding support for this in JavaScript/TypeScript. If you need this urgently,
you can open an issue on GitHub and we will prioritize it.

</Tab>
</Tabs>

As a result, you will get a nested trace with `outer` as the top level span,
where any spans created with `use_span` will be children of `outer`.
