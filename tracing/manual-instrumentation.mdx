...
Laminar.initialize({ projectApiKey: process.env.LMNR_PROJECT_API_KEY });
const foo = async (traceId: string) => {
    await observe({ name: 'foo', traceId }, async () => {
        // your code here
    });
}

const bar = async (traceId: string) => {
    await observe({ name: 'bar', traceId }, async () => {
        // your code here
    });
}
...
in our [github](https://github.com/lmnr-ai/lmnr/blob/main/frontend/lib/db/initial-data.json#L2176).
