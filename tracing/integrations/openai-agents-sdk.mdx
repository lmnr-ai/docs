---
title: OpenAI Agents SDK observability
sidebarTitle: OpenAI Agents SDK
description: "Trace OpenAI Agents SDK runs in TypeScript and Python: agent turns, tool calls, handoffs, and sub-agents in Laminar's transcript view."
---
...
The OpenAI Agents SDK lets you build agent workflows with `Agent`, `Runner.run` / `run`, function tools, and handoffs between specialists. Laminar hooks into the OpenAI Agents SDK's built-in trace processor to mirror every agent span into Laminar, plus adds the system instructions so the full prompt is visible in the trace.
...
The OpenAI Agents SDK models specialization through handoffs: a triage agent routes the user to a specialist via `handoff(other_agent)` / `handoff(otherAgent)`, and the OpenAI Agents SDK exposes each handoff as a `transfer_to_<agent>` tool call. In Laminar, the destination agent's turns and tool calls nest under the handoff span, so you can follow the full multi-agent conversation in one trace.
...
<AccordionGroup>
  <Accordion title="I don't see any traces in Laminar">
    - Confirm `LMNR_PROJECT_API_KEY` is set in the same process that runs the OpenAI Agents SDK.
