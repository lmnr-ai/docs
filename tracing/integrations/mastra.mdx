...
- Every tool invocation, with arguments and return value, including `@mastra/mcp` tools.
- Sub-agents invoked from a parent agent's tools, nested under their parent span.
- Reasoning tokens per step, rendered as `Thinking` blocks in the transcript.

<Frame caption="Mastra multi-agent trip-planner trace in Laminar's transcript view: the concierge agent delegates to flight and hotel specialists via tool calls, each sub-agent's full run expands in place, and the selected child LLM span on the right shows the model's input messages including the tool-result history.">
...
Requires `@lmnr-ai/lmnr >= 0.8.21`, `@mastra/core >= 1.28.0`, and `@mastra/observability >= 1.10.0`.

```bash
npm install @lmnr-ai/lmnr @mastra/core @mastra/observability @ai-sdk/openai
```

Swap `@ai-sdk/openai` for any provider adapter you use ([full list](https://ai-sdk.dev/providers/ai-sdk-providers)). The multi-subagent example below also uses `zod` for tool input schemas.
