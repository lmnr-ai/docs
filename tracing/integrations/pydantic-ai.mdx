...
<Frame caption="Tree view of a multi-agent Pydantic AI run. The concierge calls `book_flight` and `book_hotel` tools that spawn `flight_agent` and `hotel_agent` sub-agents, each running their own model turns and tool calls. Nesting mirrors the conversation structure.">
