...
<Tabs items={['TypeScript', 'Python']}>
  <Tab title="TypeScript">
    ...
    ```bash
    npm install @lmnr-ai/lmnr @langchain/core @langchain/openai @langchain/langgraph
    # Add other LangChain packages like @langchain/community for specific tools if needed
    ```
    ...
    <Note>
    If you are using LangChain/LangGraph with Next.js, please follow the [Next.js integration guide](/tracing/integrations/nextjs) for best practices and setup instructions for Laminar initialization.
    </Note>
    ...
    **LangGraph Example (Simple Graph):**
    ...
    // Define a new graph
    const workflow = new StateGraph<MessagesState>({ channels: MessagesState })
    ...
    async function runGraph() {
      const result = await app.invoke({ messages: [new HumanMessage("Hi there!")] });
...
- [Metadata](/tracing/metadata) - Add custom context (e.g., user IDs, environment details) to your LangChain traces.
