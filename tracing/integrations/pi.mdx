---
title: Pi observability
sidebarTitle: Pi
description: "Trace every Pi coding agent session: each prompt becomes a trace with its LLM calls and tool calls, grouped as a Laminar session."
---
...
| `LMNR_DEBUG` | none | Set to `true` to attach runs to a debugger session and write a log file. |

## Debugger sessions

Set `LMNR_DEBUG=true` and each run also joins a Laminar [debugger session](/debugger/introduction), so you can inspect and replay it. The Laminar SDK resolves the session id in this order: `LMNR_DEBUG_SESSION_ID`, then the nearest `.lmnr/debug-session.json` written by `lmnr-cli debug session new`, then a fresh id.
