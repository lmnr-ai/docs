...
Each trace carries the OpenCode session ID as its Laminar session ID, so every turn in the same OpenCode conversation shows up under one session in Laminar. Open the session to scroll through the turns in order; filter by session ID in SQL to pull a whole conversation.
