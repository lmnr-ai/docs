...
<Frame>
  <img src="/images/traces/trace-openai.png" alt="OpenAI span with prompts, tokens, and cost" />
</Frame>
...
Initialize Laminar before creating the client:
...
<Note>
Need explicit control? Pass only the modules you want with `instrumentModules: { OpenAI }`.
</Note>
...
- `Laminar.initialize(...)` patches the OpenAI SDK before you create a client.
- Each OpenAI call emits a span with prompt, response, tokens, and cost.
- Spans flow to your Laminar project automatically, ready for debugging or Playground iteration.
