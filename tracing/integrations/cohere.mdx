---
title: Cohere observability
sidebarTitle: Cohere
description: "Trace every Cohere chat, embed, and rerank call: prompts, token usage, cost, and latency, all readable in Laminar's transcript view."
---
...
```bash
pip install -U 'lmnr[all]' cohere python-dotenv
```
...
- Attach sessions, user IDs, metadata, and tags to Cohere spans via the [SDK reference](/sdk/reference).
- Wrap the functions around your Cohere calls with `observe` and mark them as [TOOL spans](/tracing/structure/span-types) so they show up in the transcript.
