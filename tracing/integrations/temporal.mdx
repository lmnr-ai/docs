---
title: Temporal observability
sidebarTitle: Temporal
description: "Trace AI agents that run as Temporal workflows in TypeScript and Python: one trace across the client, the workflow, and every activity."
---
...
Call `Laminar.initialize()` before the worker connects. It auto-instruments Temporal when `temporalio` is importable: each activity execution is wrapped in a span, and any LLM call inside it nests underneath. Pass `disable_batch=True` so the short-lived spans created inside an activity are flushed before the activity execution exits.
...
Laminar.initialize(disable_batch=True)
...
<Warning>
Set `disable_batch=True` in the worker. The worker spawns short-lived activity executions, and with the default batching span processor the internal spans created inside an activity can be dropped before they are flushed.
</Warning>
