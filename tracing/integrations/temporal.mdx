---
title: Observability for Temporal Workflows in TypeScript
...
<Note>
This integration is TypeScript-only at the time of writing. The Python Temporal integration is in progress.
</Note>
...
```bash
npm install @lmnr-ai/lmnr@latest @temporalio/client @temporalio/worker @temporalio/workflow @temporalio/activity
# or
pnpm add @lmnr-ai/lmnr@latest @temporalio/client @temporalio/worker @temporalio/workflow @temporalio/activity
```
...
export async function summarizeTopic(topic: string): Promise<string> {
  const response = await openai.chat.completions.create({
    ...
    messages: [
      ...
      { role: 'user', content: `Write a two-sentence summary of: ${topic}` },
    ],
  });
...
Because the client, workflow, and activities run in separate processes, the tree view is where the propagation pays off: the client-side `run-summary` root, the workflow lifecycle span, each activity, and the LLM and tool spans inside them all line up in a single hierarchy.
