---
title: Gemini observability
sidebarTitle: Gemini
description: "Trace every Gemini API call made with the Google Gen AI SDK: prompts, token usage, cost, and latency, all readable in Laminar's transcript view."
---
...
```bash
pip install -U 'lmnr[all]' google-genai python-dotenv
```
...
## Enrich your Gemini traces

- Attach sessions, user IDs, metadata, and tags to Gemini spans via the [SDK reference](/sdk/reference).
- Wrap the functions around your Gemini calls with `observe` and mark them as [TOOL spans](/tracing/structure/span-types) so they show up in the transcript.
