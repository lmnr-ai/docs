...
## My span exports are rejected as too large.

### Problem

Exports fail with a payload-too-large error, or time out with `DEADLINE_EXCEEDED` on a request that carries only a few dozen spans.
