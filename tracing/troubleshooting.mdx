...
1. **Import the Certifi Library**:
   ...
   - Install using pip: `pip install -U certifi`
...
## My span exports are rejected as too large (gRPC `OUT_OF_RANGE`, HTTP `413 Payload Too Large`).

### Problem

Spans stop arriving, and the exporter logs a rejection. Which error you see depends on the transport:

- **gRPC** (the default): status `OUT_OF_RANGE`, with the message `Error, decoded message length too large: found <N> bytes, the limit is: <M> bytes`.
- **HTTP** (when you set `forceHttp` / `force_http=True`): `413 Payload Too Large`, with the body `Payload too large: the request body exceeds the server's HTTP payload limit. Send smaller batches, or raise HTTP_PAYLOAD_LIMIT if you are self-hosting.`

A related symptom is `DEADLINE_EXCEEDED` on a request that carries only a few dozen spans: the batch is not rejected outright, but it is big enough to outlast the export timeout.

### Cause

The batch span processor sends a batch when it reaches a span count or when a scheduled delay elapses. Neither trigger looks at how many bytes the batch holds, and agent spans carry whole prompts, completions, and tool payloads as string attributes. A handful of large spans is enough to build a request over the ingest limit.

### Solution

Enable the byte-size flush trigger, which exports the buffer when the next span would push it past a limit:
...
<Tabs items={['TypeScript', 'Python']}>
  <Tab title="TypeScript">
    ...
    Laminar.initialize({
      projectApiKey: process.env.LMNR_PROJECT_API_KEY,
      flushBySize: true,
      maxExportBatchSizeBytes: 8 * 1024 * 1024,
    });
    ```
  </Tab>
  <Tab title="Python">
    ...
    Laminar.initialize(
        flush_by_size=True,
        max_export_batch_size_bytes=8 * 1024 * 1024,
    )
    ```
  </Tab>
</Tabs>

Lowering `maxExportBatchSize` / `max_export_batch_size` also helps and is worth trying first, but it bounds the span count rather than the payload. See [Batching](/sdk/batching) for how the two interact and what the byte limit measures.
