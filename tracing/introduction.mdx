...
Traces stream to your dashboard as they execute—near real-time by default. This makes debugging long-running agents or chained LLM calls much faster.

We still batch briefly for throughput; if you need every span immediately during development, set `disableBatch: true` in `Laminar.initialize` or toggle the setting in your project dashboard.
