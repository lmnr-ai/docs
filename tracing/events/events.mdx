---
title: Emit signals from code
---

Custom events are the most direct way to create signals. When you emit an event, it appears as a signal event in Laminar and can be analyzed alongside LLM-extracted signals (for example, `extraction.completed`, `checkout.failed`, `handoff_to_human`).
...
In the Laminar UI, open **Signals** to browse and filter signal events. Each signal has an **Events** tab with the raw stream and clusters, plus **Jobs** for backfills and **Triggers** for live analysis.

You can also:

- Extract signals from traces automatically with [LLM Signals](/tracing/events/semantic-events)
- Group similar signal events together with [Clusters](/tracing/events/clusters)
