...
LLM signals let you extract structured data that cannot be explicitly logged but can be inferred from trace data. You define a prompt that describes what to look for (for example, "Agent completes checkout process"), and the analysis LLM produces a typed signal event using the full trace as context.
...
- Use **Triggers** to keep the signal running on new traces.
...
3. Export signal events to a dataset for evaluation.
