...
Clusters group similar signal events together to help you discover patterns, debug at scale, and detect anomalies. When you have many signals (either [code-emitted events](/tracing/events/events) or [LLM signals](/tracing/events/semantic-events)), clustering reveals what is actually happening in your system.

## What Are Clusters?

Every signal event has an `attribute` field containing JSON data. Clusters group events by extracting a string value from this JSON using a Mustache template, then grouping events with similar extracted values.
...
Each cluster has member events and an automatically generated cluster name. The cluster name can be regenerated when the member events change significantly.
...
Every signal event stores its data in an `attribute` field as JSON. When you start clustering:
...
3. Signal events with similar extracted strings are grouped into clusters
