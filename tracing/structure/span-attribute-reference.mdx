...
| `lmnr.span.path` | string array | Span name path from the root. The Laminar SDKs auto-fill this; custom exporters should leave it unset unless they need explicit path control (rare). |
| `lmnr.span.ids_path` | string array | Span-id path from the root, in the same order as `lmnr.span.path`. SDK-managed; usually leave unset. |
| `lmnr.span.parent_path` | string array | Parent's `lmnr.span.path`. SDK-managed. |
| `lmnr.span.parent_ids_path` | string array | Parent's `lmnr.span.ids_path`. SDK-managed. |
