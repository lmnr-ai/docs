...
If Laminar can’t look up pricing for your provider/model, you have two options:

- **Define a price once in the UI.** Add a per-project rate under **Settings > Model costs** and Laminar prices every matching span from it, so you don't set cost attributes on every span. See [Custom model costs](/platform/custom-model-costs).
- **Attach explicit cost attributes** per span (see the example above). These take precedence over every price table, including your custom prices.
...
Laminar maintains current pricing for supported providers and keeps it up to date automatically. For a model that isn't covered, or a rate that differs from list price, define your own under [Custom model costs](/platform/custom-model-costs).
