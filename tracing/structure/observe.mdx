...
It handles span creation, activation, and ending automatically.
...
- Span input (`lmnr.span.input`) will be `[1, 2]`
...
- `input_formatter` (`Callable[P, str]`)*: a function to format the input parameters. Must take same parameters as the observed function and return a string.
- `output_formatter` (`Callable[[R], str]`)*: a function to format the output value. Must take the return value of the observed function and return a string.

\* P and R indicate that the observed function is of type `Callable[P, R]` or `Callable[P, Awaitable[R]]`.
