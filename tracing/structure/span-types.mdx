...
| `LLM` | A manual LLM call where you want Laminar to pick up token counts and cost. See [LLM cost tracking](/tracing/cost-tracking). |
...
## Setting type without the SDK

If you ship OTLP directly (no `@lmnr-ai/lmnr` / `lmnr` import), set the type as a span attribute at creation time:

```text
attributes: { "lmnr.span.type": "TOOL" }
```

See the [Span attribute reference](/tracing/structure/span-attribute-reference) for the full set of Laminar-recognized attribute keys.
