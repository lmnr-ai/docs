...
Signals are also how you hear about it without watching a dashboard. Every Signal comes with an [alert](/signals/alerts) that fires on its events, in-app, by email, or into Slack. Be clear on what that is: the alert fires on the pattern the Signal matched, not on a dollar threshold, and it reports a call that already happened rather than preventing one.
