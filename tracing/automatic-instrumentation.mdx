...
<Tip> For automatic instrumentation, import **AND** initialize Laminar before importing any instrumentable modules.</Tip>
...
<Warning> If this is not working for you, you may need to manually pass all libraries which you want Laminar to auto-instrument.
...
<Note>
For some JavaScript setups, this may be required. For example, in TS, we've seen this required for `tsx`, but not for `ts-node`.
</Note>
...
OpenTelemetry is stabilizing the support for this, so feel free to track the progress [here](https://github.com/open-telemetry/opentelemetry-js/issues/4845).
...
or [view source](https://github.com/lmnr-ai/lmnr-python/blob/main/src/lmnr/openllmetry_sdk/instruments.py).
