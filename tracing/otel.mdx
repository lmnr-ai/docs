...
OpenTelemetry covers tracing, metrics, and logs. Laminar focuses on tracing today; limited log ingestion
is available for some SDK integrations and can be queried in the SQL Editor. For more details about
tracing in Laminar, and a quick intro to the core concepts, see the [Tracing](/tracing/introduction) page.
...
which has three on-the-wire variants:

- Protobuf over gRPC (commonly known as OTLP/gRPC)
- Protobuf over HTTP (commonly known as OTLP/HTTP/proto)
- JSON over HTTP (commonly known as OTLP/HTTP/json)

Laminar's backend (both cloud and self-hosted) supports all three. OTLP/gRPC is recommended for performance and reliability; OTLP/HTTP/json is useful for runtimes that can't ship a protobuf encoder, such as browser code.

Here's a quick comparison of the formats that Laminar supports:

|  | OTLP/gRPC | OTLP/HTTP/proto | OTLP/HTTP/json |
| --- | --- | --- | --- |
| Supported | ✅ | ✅ | ✅ |
| Recommended | ✅ | ❌ | only when proto isn't an option |
| Underlying protocol | gRPC over HTTP/2 | HTTP/1.1 | HTTP/1.1 |
| Encoding format | protobuf | protobuf | JSON |
| `Content-Type` | n/a | `application/x-protobuf` | `application/json` |
| Base URL for Laminar cloud | `https://api.lmnr.ai` | `https://api.lmnr.ai` | `https://api.lmnr.ai` |
| Port at Laminar cloud | `8443` | `443` | `443` |
| Default port for self-hosted backend | `8001` | `8000` | `8000` |
| Path | `/v1/traces` [1] | `/v1/traces` [1] | `/v1/traces` [1] |
...
[project API key](/tracing/introduction#getting-started).
...
For the full list of attribute keys Laminar reads (span shape, trace-association, LLM telemetry, GenAI semconv), see [Span attribute reference](/tracing/structure/span-attribute-reference).
