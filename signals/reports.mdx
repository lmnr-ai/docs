...
Reports can post to a single Slack channel per report. Slack messages include the same per-signal counts, AI summary, and noteworthy events as the email. Connect Slack from **Workspace > Integrations** (see [Slack integration](/platform/integrations)) or from the Reports page.
