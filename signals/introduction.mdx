...
<Frame caption="A Signal's events tab: each row is one detected event linked back to a trace">
  <img src="/images/signals/signal-detail.png" alt="Signal detail view with events list and per-event payload" />
</Frame>
...
{/* DIAGRAM PLACEHOLDER: full pipeline overview.
    Left to right: [Trace arrives] → [Trigger match] → [Trace compression] → [Signal agent investigates: search spans / fetch full spans] → [Findings submitted] → [Signal events] → fan-out to [Alerts] and [Clusters].
    One box per stage, single flow line, with the agent stage drawn as a loop (investigate ↔ trace) before it exits to findings. */}
...
{/* DIAGRAM PLACEHOLDER: agent investigation loop.
    Center: [Signal agent]. Inputs: [Your prompt + schema] and [Compressed transcript].
    Loop arrows from the agent to [Search span content] and [Fetch full span by id], both pointing back to the agent.
    Exit arrow: [Submit findings] → zero, one, or many findings. */}
