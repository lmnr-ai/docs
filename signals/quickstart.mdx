...
5. **Trigger**: when the Signal evaluates a new trace. Leave the default (**Root span finished**) unless the work is spread across services, then pick **Span name**.
...
<Frame caption="Signal Settings > General: name, prompt, output schema, trigger, filters, and sampling">
...
- **Root span finished** (the default) fires when the run's root span ends, so the Signal sees the complete trace.
- **Span name** fires when a span with one of the names you list ends. Use it when the work is spread across services.

Open the Signal's **Settings > General** section to change the trigger, then save.

### Filters

Filters decide whether a triggered Signal actually runs. They are properties of the whole trace, and every one of them must match.
