...
- An agent or LLM app whose traces land in Laminar. The example below uses the [Claude Agent SDK](/integrations/claude-agent-sdk) to produce traces quickly.
- On a self-hosted deployment, one [LLM profile](/platform/llm-profiles) in the workspace. Each Signal picks a profile and one of its models to run on. On Laminar Cloud there is nothing to configure.
...
| **Span names**   | array  | Any span name anywhere in the trace.                                  |
| **Span tags**    | array  | Any [tag](/tracing/structure/tags) attached to any span in the trace. |
...
<Note>
By default, Signals will notify the creator of critical events through the platform notifications panel and email. Open the Signal's **Settings > Alerts** section to change channels, connect Slack, or add additional filters. See [Alerts](/signals/alerts).
</Note>

## 4. Manage the Signal

A Signal can evaluate new traces as they arrive, or a historical slice. Open a Signal and click **Settings** to manage both from a sidebar with four sections: **General** (definition, trigger, and filters), **Versions** (the history of the Signal's definition), **Alerts**, and **Backfill**. Every evaluation the Signal performs is listed on the **Runs** tab, next to **Events** and **Settings**.
...
Runs appear on the **Runs** tab with a status (Pending, Processing, Completed, Failed), a source that distinguishes backfilled Runs from the ones a live trigger produced, and the token cost of the evaluation. Each Run that detected the Signal links to the event it created.

<Frame caption="The Runs tab: one row per evaluated trace with its source, status, cost, and the event it produced">
  <img
    src="/images/signals/signal-activity.png"
    alt="Signal runs table listing runs with status, source, event, clusters, cost, and event id"
  />
</Frame>
...
- Use the clusters view above the events table to see similar events grouped together. See [Clusters](/signals/clusters).
- Query the raw events in the [SQL Editor](/platform/sql-editor) via the `signal_events` table.

## Querying signal events in SQL
...
Events are also attached to the traces they came from, as `signal_events` and `clusters` columns on the `traces` table. Read them from there when the row you want back is a trace: see [Signal events on traces](/platform/sql-editor#signal-events-on-traces).
