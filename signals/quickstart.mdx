...
To write your own, click **Create signal**. If you would rather script it, or hand the job to an AI coding agent, the same Signal can be created from your terminal with [`lmnr-cli signal create`](/signals/cli).
...
5. **Trigger**: when this Signal is evaluated on a new trace, either once the trace finishes or when a named span arrives. Add filters to narrow which traces it runs against.
...
Triggers run the Signal automatically on new traces. **This is what feeds live dashboards and Alerts.**

The **trigger** is when Laminar starts looking at a trace. Pick one of two moments:

- **Root span finished**: the trace is complete. The default, and the right choice for most agents.
- **Span name**: a span you name has arrived. Use this when the trace keeps growing after the root span closes, for example when a background worker adds spans to the same trace minutes later.

**Filters** are which traces to look at. Once the trigger fires, the Signal runs only on traces matching all of the filters: only errored runs, only runs above a token count, only runs that used a particular tool. With no filters it runs on every triggered trace.

- Open the Signal's **Settings > General** section and find **Trigger** (or configure it while creating the Signal).
- Pick the moment, then add filters to narrow which traces qualify.
- Save. Every matching trace now produces a Run; runs that detect the Signal produce events.

**Processing mode** sets how fast runs are processed: batch (results within several hours) or realtime (results in minutes, billed as two signal runs). Use realtime for Signals that feed an alert someone acts on. All of this is also configurable from the [CLI](/signals/cli#triggers).
...
<Frame caption="The Active switch at the top of Settings > General pauses and resumes the Signal">
  <img src="/images/signals/signal-active-switch.png" alt="Signal settings General section with the Active switch at the top of the form" />
</Frame>
...
<CardGroup cols={2}>
  ...
  <Card title="Signals CLI" href="/signals/cli" icon="terminal">
    Create, update, and delete Signals from your terminal or a coding agent.
  </Card>
