...
### Start from traces when you want traces back

Signal events are also attached to the traces they came from, as a `signal_events` column on the `traces` table. Unnest it and you get trace columns and event fields side by side, with no join:
...
Each tuple has `event_id`, `signal_id`, `severity`, and `payload`. Traces no Signal fired on have an empty array, so `notEmpty(signal_events)` and `empty(signal_events)` are the cheap way to count coverage. The [clusters column](/signals/clusters#querying-clusters-with-sql) on `traces` works the same way.

Query `signal_events` directly, as below, when the row you want back is an event.
