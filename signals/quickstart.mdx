...
<Frame caption="Create Signal drawer: name, template, prompt, and output schema on top, trigger and filters below">
  <img src="/images/signals/create-signal-empty.png" alt="Empty Create Signal drawer showing the trigger options and the default total tokens filter" />
</Frame>
...
5. **Trigger**: when the Signal evaluates a new trace. Leave the default (**When a trace finishes**) unless the work is spread across services, then pick **When a specific span finishes** and list the span names.
6. **Filters**: which traces actually run. The default is total tokens > 1000. Add more; they are AND-combined.
7. **Sampling** (optional): cap how many matching traces are evaluated. Useful when a high-volume filter would otherwise fan out across every trace.

<Frame caption="Create Signal drawer filled in from the Task template: the prompt and output schema come prefilled">
  <img src="/images/signals/create-signal-filled.png" alt="Create Signal drawer populated with a name, the Task template, a prompt, and two output schema fields" />
</Frame>
...
A Signal can evaluate new traces as they arrive, or a historical slice. **Live evaluation is what feeds dashboards and Alerts.** Open a Signal and click **Settings** to manage both from a sidebar with four sections: **General** (definition, trigger, and filters), **Alerts**, **Backfill**, and **Activity**.

<Frame caption="Signal Settings > General: prompt, output schema, trigger, filters, and sampling">
  <img src="/images/signals/signal-settings.png" alt="Signal settings General section with the trigger options and the filters bar" />
</Frame>

### Triggers

The trigger is the moment Laminar reads the trace. Every Signal has exactly one.

- **When a trace finishes** (the default) fires when the run's root span ends, so the Signal sees the complete trace.
- **When a specific span finishes** fires when a span with one of the names you list ends. Use it when the work is spread across services.

Open the Signal's **Settings > General** section to change the trigger, then save.

### Filters

Filters decide whether a triggered Signal actually runs. They are properties of the whole trace, and every one of them must match. You build them in the same filter bar you use on the traces table.

A new Signal gets **total tokens > 1000** by default. You can also filter on:

- **Total tokens**: comparisons like `>` `1000`
- **Status**: `error` or `success`
- **Span names**: whether a name appears anywhere in the trace

Clear the filters to run on every trace the trigger fires for.
...
<Frame caption="The Active switch at the top of Settings > General; flipped off, the Signal reads Inactive">
  <img src="/images/signals/signal-active-switch.png" alt="Signal settings General section with the Active switch flipped off and the signal marked Inactive" />
</Frame>
...
<Frame caption="The Signals list groups paused Signals under a Disabled section">
  <img src="/images/signals/signals-list-disabled.png" alt="Signals list with an Active section and a Disabled section, the disabled Signal card dimmed with a Disabled badge" />
</Frame>
