...
A trigger decides two things. First, **when** to evaluate a trace: either once the trace finishes (the default, and the right choice for most agents) or when a span with a name you pick arrives. The second option matters when spans keep arriving after the root span closes, as in a distributed trace where a background worker reports back later. Second, **whether** a trace is worth a run, which is what the filters do: only errored runs, only runs above a token count, only runs that touched a particular tool.

- Open the Signal's **Settings > General** section and find **Trigger** (or configure it while creating the Signal).
- Pick when it should be evaluated, then add filters to narrow which traces qualify. Filters are AND-combined, and with none the Signal runs on every triggered trace.
- Save. Every matching trace now produces a Run; runs that detect the Signal produce events.

Each trigger also has a **Processing mode**: batch (results within several hours) or realtime (results in minutes, billed as two signal runs). Use realtime for Signals that feed an alert someone acts on. The same trigger can be configured from the [CLI](/signals/cli#triggers).
