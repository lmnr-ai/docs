...
Creating a new Signal auto-creates alerts so you start receiving notifications immediately:
...
The alerts default to in-app notifications, and the Signal's creator is subscribed by email. To change channels or route an alert to Slack, open **Project Settings > Alerts** and edit it.
...
Every alert posts to the in-app notification center by default. The creator is also subscribed by email when a Signal creates its default alerts; Slack and any additional email subscriptions are configured per alert.

### Slack

Slack messages contain the signal name, the extracted payload fields, and a link to the trace. Connect Slack once per workspace from **Workspace > Integrations** (see [Slack integration](/platform/integrations)) or from the Alerts settings page.
...
<Frame caption="Each alert card has an on/off switch; disabled alerts render dimmed">
  <img src="/images/signals/alerts-toggle.png" alt="Project alerts settings with three alert cards, one toggled off and dimmed" />
</Frame>
