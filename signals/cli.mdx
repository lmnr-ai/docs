...
`--filter` takes JSON `{"column":"...","operator":"...","value":"..."}` and is repeatable. Columns: `total_token_count`, `status`, `span_names`, `tags`. Pass it on create to replace the default. Array filters (`span_names`, `tags`) take `"operator": "includes"` or `"not_includes"` and a list of strings; `includes` is OR (any listed value present).

<Note>
`--span-name` is the trigger flag and is OR if you pass it more than once: the Signal fires when any of those spans ends. `span_names` is a filter column: after the trigger fires, it checks whether any listed name appears anywhere in the trace. Empty names are ignored on both.
</Note>
...
Options:
  ...
  --span-name <name>         Span name to trigger on (repeatable, OR). Requires --trigger span-name
