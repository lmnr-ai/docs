...
Laminar offers two ways to run evaluations:

### evaluate() Function (This Guide)
- **Best for**: Quick evaluations, prototyping, getting started
- **Pros**: Simple setup, automatic handling of lifecycle, built-in error handling
- **Cons**: Less flexibility, fixed workflow structure

### Evaluation SDK
- **Best for**: Production systems, complex workflows, custom integrations
- **Pros**: Full programmatic control, custom error handling, flexible async operations
- **Cons**: More setup required, manual lifecycle management

For detailed information on using the SDK approach, see our [Evaluation SDK guide](/evaluations/sdk).
