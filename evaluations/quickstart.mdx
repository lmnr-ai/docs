...
npx lmnr@latest eval my-first-evaluation.ts
```
...
Pick a data type (usually `String` or `Float64`) and save. The expression is evaluated per row on `evaluation_datapoints`. For more JSON parsing functions, see the SQL Editor guide at `/platform/sql-editor`.
