...
## Track progress, then drill into a run

The chart above answers *did the score move*. The run view answers *why*: it's a split view with every datapoint and its scores on the left and the selected datapoint's full trace on the right, so you go from an aggregate number to the exact model call behind it in one click.
