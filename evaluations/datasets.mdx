...
After an evaluation completes, you can turn any subset of its rows into a new dataset. From the **SQL editor**, write a query against `evaluation_datapoints` or `spans`, then click **Export to Dataset**. This is the workflow for [calibrating LLM-as-a-judge against human scores](/evaluations/human-evaluators#validating-llm-as-a-judge-evaluators): query the human evaluator spans, export, then run an evaluation against the exported dataset using the human score as `target`.
...
<CardGroup cols={2}>
  ...
  <Card title="Human evaluators" href="/evaluations/human-evaluators" icon="user-check">
    Collect human scores into a dataset and use them to validate LLM judges.
  </Card>
