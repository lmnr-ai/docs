...
<Note>
Spans are the one thing the evaluation API does not carry: the `EVALUATION` / `EXECUTOR` / `EVALUATOR` spans behind per-row transcripts are emitted over OpenTelemetry. Datapoints written over HTTP still get `data`, `target`, `executorOutput`, and `scores`, which is everything the list view, progression chart, and comparison UI need. For transcripts too, emit spans with any OTel SDK (see [OpenTelemetry](/tracing/otel)) and pass the same `traceId` on the datapoint.
</Note>
