...
| Rename or re-tag the run | none yet | [`POST /v1/evals/{eval_id}`](#update-an-evaluation) |
...
A long-running evaluation often doesn't know its own name or final state up front: the run is created when work starts, and only later do you know the model version, the git SHA, or whether it completed cleanly. [`POST /v1/evals/{eval_id}`](#update-an-evaluation) updates an evaluation's `name` and `metadata` after the fact. There is no SDK method for this yet, so call the endpoint directly.
...
### Attaching traces to datapoints

Spans are the one thing this API does not carry. The `EVALUATION` / `EXECUTOR` / `EVALUATOR` spans behind per-row transcripts travel over OpenTelemetry, not over these endpoints, and nothing links a datapoint to a trace implicitly. A datapoint gets a trace only because you passed its `traceId`.

The SDKs hide this: `Laminar.getTraceId()` / `Laminar.get_trace_id()` read the active span, so the walkthrough above can hand the id straight to `createDatapoint`. Without an SDK you capture it yourself from whatever emits your spans, then send it on the datapoint.

Laminar stores trace ids as UUIDs while OpenTelemetry uses 32 hex characters. Both forms are accepted on `traceId`, and the hyphenated UUID is what you'll see in the UI and in SQL:

```
OpenTelemetry   4bf92f3577b34da6a3ce929d0e0e4736
Laminar UUID    4bf92f35-77b3-4da6-a3ce-929d0e0e4736
```

Datapoints written without a `traceId` are still complete rows: `data`, `target`, `executorOutput`, and `scores` are everything the list view, progression chart, and side-by-side comparison read. Only the per-row transcript drill-down needs a trace. To get that too, emit your spans with any OTel SDK (see [OpenTelemetry](/tracing/otel)) and pass that trace's id here.
...
| `points[].traceId` | UUID | no | Trace that owns this row, so clicking it opens that trace's transcript. You supply this: nothing links a datapoint to a trace on its own. Omit it and the row simply has no trace attached. |
