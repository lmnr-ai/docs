...
Fields you leave out are unchanged, so you can rename a run without touching its metadata. When you do send `metadata`, it replaces the stored object wholesale rather than merging: include the keys you want to keep, as the examples above do. The group cannot be changed, because only runs sharing a group can be [compared](/evaluations/comparing-runs) and existing datapoints already carry the group they were written with.
