...
| `flushBySize` | `boolean` | `false` | Also flush a batch when its payload would exceed `maxExportBatchSizeBytes`. See [Batching](/sdk/batching) |
| `maxExportBatchSizeBytes` | `number` | `33554432` | Approximate max bytes per export batch. Only used when `flushBySize` is true |
