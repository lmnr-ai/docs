...
| `flush_by_size` | `bool` | `False` | Also flush a batch when its payload would exceed `max_export_batch_size_bytes`. See [Batching](/sdk/batching) |
| `max_export_batch_size_bytes` | `int` &#124; `None` | `None` | Approximate max bytes per batch (defaults to 32 MiB). Only used when `flush_by_size` is True |
