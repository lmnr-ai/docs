---
title: Claude Agent SDK observability
sidebarTitle: Claude Agent SDK
description: "Trace agents built on the Claude Agent SDK in TypeScript and Python, including subagent runs, tool calls, token usage, and cost."
---
...
The Claude Agent SDK supports Claude models hosted on Amazon Bedrock. Laminar traces work out of the box: follow Anthropic's [Amazon Bedrock guide](https://code.claude.com/docs/en/amazon-bedrock) to configure your AWS credentials and the SDK environment variables, and additionally set `AWS_REGION` to the region you're targeting.

```bash
export CLAUDE_CODE_USE_BEDROCK=1
export AWS_REGION=us-west-2
# plus the AWS credentials from the Anthropic Bedrock guide
```

<Note>
`AWS_REGION` is required for Laminar tracing to work correctly on Bedrock.
</Note>

Initialize Laminar exactly as shown in [Getting started](#getting-started); no other code changes are needed.
...
<AccordionGroup>
  <Accordion title="I don't see any traces in Laminar">
    - Confirm `LMNR_PROJECT_API_KEY` is set in the same process that runs the Claude Agent SDK.
...
<CardGroup cols={2}>
  ...
  <Card title="OpenCode" href="/integrations/opencode">
    Running the Claude Agent SDK alongside OpenCode? Trace both here.
  </Card>
