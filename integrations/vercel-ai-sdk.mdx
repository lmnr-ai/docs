---
title: Vercel AI SDK observability
sidebarTitle: Vercel AI SDK
description: "Trace Vercel AI SDK calls in Next.js and Node.js: model turns, tool calls, streaming, sub-agents, token usage, and cost."
---
...
The same `telemetry` block also takes `functionId`, which names the call's span:

```typescript
const { text } = await generateText({
  model: openai('gpt-5-mini'),
  prompt: 'Draft a release note.',
  telemetry: { functionId: 'release-notes' },
});
```
...
## Attach metadata, sessions, user IDs, and tags

On AI SDK v7, per-call context goes in `runtimeContext`. The AI SDK only sends a `runtimeContext` key to telemetry when you opt it in with `telemetry.includeRuntimeContext`, so list every key you want in Laminar there.
...
Laminar reads the opted-in keys like this:

| Key | Becomes |
| --- | --- |
| `userId` | The trace's [user ID](/tracing/structure/user-id) |
| `sessionId` | The trace's [session ID](/tracing/structure/sessions) |
| `tags` | [Tags](/tracing/structure/tags) on the call's span. Must be an array of strings. |
| Any other key | A [metadata](/tracing/structure/metadata) key-value pair on the trace |

Keys you leave out of `includeRuntimeContext` stay in your application and never reach Laminar, so you can keep request-scoped values in `runtimeContext` without exporting them.

<Note>
Reading `runtimeContext` requires `@lmnr-ai/lmnr` 0.8.49 or later.
</Note>
...
const result = await generateText({
  ...
  telemetry: {
    ...
    includeRuntimeContext: { env: true },
  },
});
...
<Note>
On AI SDK v5 and v6, replace the `runtimeContext` and `telemetry` blocks with `experimental_telemetry: { isEnabled: true, tracer: getTracer(), functionId: 'travel-assistant', metadata: { env: 'production' } }`.
</Note>
...
See the full [observe reference](/tracing/structure/observe-decorator) for session IDs, user IDs, metadata, and tags on the parent span. For per-call context on AI SDK v7, use [`runtimeContext`](#attach-metadata-sessions-user-ids-and-tags).
...
If a call passed `functionId` through `experimental_telemetry`, or you want to turn tracing off for it, use the `telemetry` block instead. Metadata moves to `runtimeContext`, with each key opted in through `telemetry.includeRuntimeContext` (see [Attach metadata, sessions, user IDs, and tags](#attach-metadata-sessions-user-ids-and-tags)).
...
<CardGroup cols={2}>
  ...
  <Card title="OpenAI" href="/integrations/openai">
    Mixing the AI SDK with the OpenAI SDK directly? Trace it here.
  </Card>
  <Card title="Anthropic" href="/integrations/anthropic">
    Mixing the AI SDK with the Anthropic SDK directly? Trace it here.
  </Card>
