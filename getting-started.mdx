...
- Transcript view is intentionally high-signal: it focuses on LLM and TOOL spans. Ensure the key work in the system is represented with LLM/TOOL spans (or clearly parented under them) so traces are easy to scan.
...
- Custom LLM providers or custom tools:
  - Create spans with spanType LLM/TOOL so Transcript view and UI render correctly.
...
- Verification checklist: how to validate traces in the UI (root span exists, children nested, Transcript view is readable, filters work, tags show)
...
The fastest way to get started: run the CLI once, then let your coding agent do the rest.

<Steps>
  <Step title="Run setup at the project root">
    ```bash
    npx lmnr-cli@latest setup
    ```
    `setup` authenticates you, writes a new project API key to your env file, links the directory, and installs the [Laminar skill](https://github.com/lmnr-ai/lmnr-skills) into your project (`.claude/`, `.cursor/`, `.codex/`, or `.agents/`) so your coding agent knows how to instrument with Laminar. See the [CLI page](/platform/cli) for details.
  </Step>
  <Step title="Ask your coding agent to instrument your project">
    With the skill installed, prompt Claude Code, Cursor, or Codex with something like `Instrument this repo with Laminar`. It picks the right [integration](/tracing/integrations/overview), adds initialization at the correct entrypoint, and wires up the SDK.
  </Step>
  <Step title="Run your project">
    Run your app as you normally would so it produces a trace.
  </Step>
  <Step title="Verify traces are landing">
    ```bash
    lmnr-cli sql query "SELECT * FROM traces ORDER BY start_time DESC LIMIT 1" --json
    ```
  </Step>
  <Step title="View your traces in the browser">
    Open your project in the [dashboard](https://www.lmnr.ai/projects) to read the full transcript.
  </Step>
</Steps>

Prefer Cursor completions over the skill? Run `lmnr-cli setup` for the key, then `pip install 'lmnr[all]' && lmnr add-cursor-rules` to write `.cursor/rules/laminar.mdc`. Reload Cursor (`Cmd+Shift+P` → "Reload Window") to activate.
...
→ [Tracing docs](/tracing/introduction) · [Hosting options](/hosting-options)
...
Add Laminar to your app and start capturing LLM and tool spans.

→ [Tracing docs](/tracing/introduction)

### Hosting

Choose between Laminar Cloud and self-hosting (and how to configure base URLs).
