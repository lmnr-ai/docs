...
- Optional (if I ask for evals): add or migrate evaluations using Laminar evaluate(), and provide runnable commands (lmnr eval / npx lmnr@latest eval) that work with the repo’s tooling.
...
- If I ask: include guidance for migrating evaluations to Laminar evaluate() and running them via lmnr eval / npx lmnr@latest eval.

Now, here is my repo context + code:
[PASTE HERE]
`;

Laminar’s entry point is tracing: once your app is instrumented, production runs become traces you can explore in the platform.

Start here:

<CardGroup cols={2}>
  <Card title="Tracing" href="/tracing/introduction">
    Add Laminar to your app and start capturing LLM/tool spans.
  </Card>
