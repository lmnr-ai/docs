...
- First, determine the repo’s package manager and use it (don’t invent a new one):
  - TypeScript: if pnpm-lock.yaml -> use 'pnpm add @lmnr-ai/lmnr'; if yarn.lock -> 'yarn add @lmnr-ai/lmnr'; if package-lock.json -> 'npm add @lmnr-ai/lmnr'; if bun.lockb -> 'bun add @lmnr-ai/lmnr'.
...
export const COPY_PROMPT_BUTTON_STYLE = {
  display: 'flex',
  alignItems: 'center',
  ...
  padding: '12px 14px',
  borderRadius: '12px',
  border: '1px solid rgba(237, 110, 64, 0.35)',
  background: 'rgba(237, 110, 64, 0.10)',
...
export const COPY_PROMPT_LIST_STYLE = {
  display: 'flex',
...
export const COPY_PROMPT_ITEM_STYLE = {
  display: 'flex',
  flexDirection: 'column',
  gap: '10px',
};

export const COPY_PROMPT_BADGE_STYLE = {
  flex: '0 0 auto',
  display: 'inline-flex',
  alignItems: 'center',
  ...
  borderRadius: '999px',
  border: '1px solid rgba(237, 110, 64, 0.35)',
  background: 'rgba(237, 110, 64, 0.16)',
  fontSize: '0.8rem',
  fontWeight: 700,
...
<div style={COPY_PROMPT_LIST_STYLE}>
  <div style={COPY_PROMPT_ITEM_STYLE}>
    <button
      type="button"
      onClick={() => navigator.clipboard.writeText(LAMINAR_INSTRUMENTATION_PROMPT.trim())}
      style={COPY_PROMPT_BUTTON_STYLE}
    >
      <span>Have an existing pipeline/product and want to incorporate Laminar?</span>
      <span style={COPY_PROMPT_BADGE_STYLE}>Copy</span>
    </button>
  ...
  <div style={COPY_PROMPT_ITEM_STYLE}>
    <button
      ...
      style={COPY_PROMPT_BUTTON_STYLE}
    >
      <span>Just want a basic install + auto-instrumentation?</span>
      <span style={COPY_PROMPT_BADGE_STYLE}>Copy</span>
    </button>
  ...
  <div style={COPY_PROMPT_ITEM_STYLE}>
    <button
      ...
      style={COPY_PROMPT_BUTTON_STYLE}
    >
      <span>Migrating from another observability platform?</span>
      <span style={COPY_PROMPT_BADGE_STYLE}>Copy</span>
    </button>
...
Sign up at [laminar.sh](https://laminar.sh), create a project, and grab your API key from project settings.
