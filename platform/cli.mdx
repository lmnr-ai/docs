...
3. Mints a fresh project API key for your application's SDK and writes `LMNR_PROJECT_API_KEY=...` to your env file (an existing `.env.local` if you have one, otherwise `.env`).
...
`LMNR_DASHBOARD_URL`, `LMNR_BASE_URL`, and `LMNR_HTTP_PORT` are honored as fallbacks. The CLI also reads them from a `.env` or `.env.local` file in the current directory, so a self-hosted config committed to your repo applies to every command without exporting anything.
...
These commands are the operational layer for the [Debugger](/debugger/workflow): you run the agent with `LMNR_DEBUG=true`, then use these to name the session and write per-trace notes. Like the other commands, they run against the linked project.
