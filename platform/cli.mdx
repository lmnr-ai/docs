...
The `lmnr-cli` is a command-line tool for onboarding a project, querying Laminar data, managing datasets and Signals, and annotating debug sessions from your terminal. It is the fastest way to wire up a new project, pull traces into a shell pipeline, script dataset maintenance, or connect an AI coding agent that can shell out, all without leaving the command line.

<Note>
The CLI is a standalone npm package (`lmnr-cli`) and ships independently from the `@lmnr-ai/lmnr` SDK. You do not need the SDK installed to onboard a project, query data, or manage datasets and Signals.
</Note>
## Install
...
For a self-hosted deployment, point the CLI at your instance. `login` talks to your frontend (the auth issuer); the data commands (`sql`, `dataset`, `signal`, `project`, `debug`) talk to your API.
...
Every project command (`sql`, `dataset`, `signal`, `project`, `debug`) resolves which project it targets from the directory you run in. `lmnr-cli setup` writes a `.lmnr/project.json` link file at the project root (it holds the project id and display details, never secrets). Commands then resolve the project in this order:
...
List, create, update, and delete [Signals](/signals/introduction) without opening the dashboard. Like `sql`, these commands run against the linked project.

```bash
# List signals in the project
lmnr-cli signal list

# Create a signal from a prompt and a payload schema
...
# Update one field (omitted flags keep their stored value)
lmnr-cli signal update "Refund requests" --sample-rate 10

# Pause a signal instead of deleting it
lmnr-cli signal update "Refund requests" --disabled

# Delete a signal, its triggers, its alerts, and its events
lmnr-cli signal delete "Refund requests"
```

`get`, `update`, and `delete` accept a signal id or a name. See [Signals CLI](/signals/cli) for the payload schema rules and the trigger model.
