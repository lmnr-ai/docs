...
Create, inspect, update, and delete [Signals](/signals/introduction) without leaving the terminal. A Signal created this way is the same object the UI's **Create signal** drawer produces, so it stays editable in the app.
...
Three independent flags decide which traces a Signal reads: `--trigger` (when it is evaluated), `--filter` (whether it runs), and `--mode` (how the run is processed). See [Signals CLI](/signals/cli) for the full reference.
