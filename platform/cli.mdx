...
Re-running `setup` in the same directory reuses the linked project and keeps a working key in place: if a valid key for this project is already configured (in your environment or env file), it leaves it untouched. It mints a fresh one only when no key is set, or the existing one is invalid or revoked. Keys stay visible (and revocable) in **Settings → Project API Keys**.
...
For a self-hosted deployment, point the CLI at your instance. `login` talks to your frontend (the auth issuer); the data commands (`sql`, `dataset`, `project`, `trace`, `debug`) talk to your API and take `--base-url` separately:
...
`LMNR_FRONTEND_URL`, `LMNR_BASE_URL`, and `LMNR_HTTP_PORT` are honored as fallbacks. The CLI also reads them from a `.env` or `.env.local` file in the current directory, so a self-hosted config committed to your repo applies to every command without exporting anything.
