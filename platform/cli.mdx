...
3. Mints a fresh project API key and writes `LMNR_PROJECT_API_KEY=...` to `./.env`.
...
- **Your user account**, via `lmnr-cli login`. Used by `sql`, `dataset`, and `project`. These commands run as the signed-in user, scoped to a project (see [Directory-scoped projects](#directory-scoped-projects)).
- **A project API key**, via `--project-api-key` or `LMNR_PROJECT_API_KEY`. Used by `debug` and `trace`.
...
`debug` and `trace` authenticate with a project API key. Grab one from **Settings → Project API Keys**, then set it once:

```bash
export LMNR_PROJECT_API_KEY=<your-key>
```

Alternatively, pass `--project-api-key <key>` on those commands. `lmnr-cli setup` writes this key to `./.env` for you.

## Directory-scoped projects

`sql`, `dataset`, and `project` resolve which project they target from the directory you run in, Vercel- and Supabase-style. `lmnr-cli setup` writes a `.lmnr/project.json` link file at the project root (it holds the project id and display details, never secrets). Commands then resolve the project in this order:

1. `--project-id <id>` flag.
2. The nearest `.lmnr/project.json`, walking up from the current directory.

So once a directory is linked, every `sql` and `dataset` command in it (or any subdirectory) targets that project with no extra flags. Override per command with `--project-id`.
...
Run ClickHouse SQL against your project's spans, traces, signal events, and more. `sql` runs as the signed-in user against the linked project, so `lmnr-cli login` plus a linked directory (or `--project-id`) is all you need:
...
List, push, pull, and create datasets from files on disk. Supported formats: `.jsonl`, `.json`, `.csv`. Like `sql`, these commands run as the signed-in user against the linked project.
...
These commands are the operational layer for the [Debugger](/platform/debugger): you run the agent with `LMNR_DEBUG=true`, then use these to name the session and write per-trace notes. They authenticate with a project API key (`--project-api-key` or `LMNR_PROJECT_API_KEY`).
...
- **Stable noun-verb command surface** (`sql query`, `dataset push/pull/list/create`, `project list`, `trace append-note`, `debug session set-name/summary`), so commands are easy for an agent to compose.
