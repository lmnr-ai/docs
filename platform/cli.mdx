...
The `lmnr-cli` is a command-line tool for onboarding a project, querying Laminar data, managing datasets, Signals, and LLM profiles, and annotating debug sessions from your terminal. It is the fastest way to wire up a new project, pull traces into a shell pipeline, script dataset maintenance, or connect an AI coding agent that can shell out, all without leaving the command line.
...
For a self-hosted deployment, point the CLI at your instance. `login` talks to your frontend (the auth issuer); the data commands (`sql`, `dataset`, `signal`, `llm-profile`, `project`, `debug`) talk to your API.
...
Every project command (`sql`, `dataset`, `signal`, `llm-profile`, `project`, `debug`) resolves which project it targets from the directory you run in. `lmnr-cli setup` writes a `.lmnr/project.json` link file at the project root (it holds the project id and display details, never secrets). Commands then resolve the project in this order:
...
When you do not want to write the SQL yourself, ask [Laminar Agent](/platform/laminar-agent) a question in plain language. It runs read-only SQL and reads trace context for you, then answers grounded in your project's data:

```bash
lmnr-cli ask "What failed in the most recent run for my dev-claude session?"
```

Like the other commands, `ask` runs against the linked project and authenticates as the signed-in user, so `lmnr-cli login` plus a linked directory (or `--project-id`) is all you need.

```bash
lmnr-cli ask "List the 5 slowest traces in the last hour and explain the slowest one"
lmnr-cli ask "How much did we spend on LLM calls today, broken down by model?" --project-id <uuid>
```

### Continue a conversation

Each answer prints a conversation id after it. Pass that id to `--conversation` on your next question to continue the same conversation, so the agent keeps the earlier context and you can ask follow-ups:

```bash
lmnr-cli ask "What failed in the most recent run for my dev-claude session?"
# ...answer, then: continue with: lmnr-cli ask "<question>" --conversation <id>
lmnr-cli ask "and which model did it use?" --conversation <id>
```

With `--json`, the conversation id comes back as `conversationId` in the output (`{ answer, conversationId, tools }`), so a script can capture it and thread it through a multi-turn exchange:

```bash
lmnr-cli ask "summarize the most expensive trace today" --json
```

See [Laminar Agent](/platform/laminar-agent) for the full list of what it can do and the other surfaces (MCP, Slack) it runs on.

## Manage Signals

Create, inspect, update, and delete [Signals](/signals/introduction) from the terminal.
...
See [Signals CLI](/signals/cli) for the full reference, including the `--llm-profile-id` and `--model` pair that routes a Signal through an [LLM profile](/platform/llm-profiles) on self-hosted deployments.

## Manage LLM profiles

An [LLM profile](/platform/llm-profiles) bundles a provider, its credentials, and the models you want into one workspace-level entry. `llm-profile` is full CRUD over them, so you can provision providers from a script instead of the settings page.
...
lmnr-cli llm-profile create "Staging OpenAI" \
  --provider openai_responses --api-key sk-... \
  --model gpt-5-mini --model gpt-4.1-mini

lmnr-cli llm-profile update <profile-id> --model gpt-5
lmnr-cli llm-profile delete <profile-id>
```

Secrets are write-only: reads return a mask, and an update that omits a secret keeps the stored value. See [LLM profiles from the CLI](/platform/llm-profiles#manage-profiles-from-the-cli) for the per-provider flags and the update rules.

## Manage datasets

Create, list, inspect, rename, delete, push, and pull datasets. File operations support `.jsonl`, `.json`, and `.csv`. Like `sql`, these commands run against the linked project.

```bash
# List all datasets in the project
lmnr-cli dataset list --json

# Inspect, rename, or delete a dataset by its canonical ID
lmnr-cli dataset get <dataset-id> --json
lmnr-cli dataset update <dataset-id> --name renamed-dataset
