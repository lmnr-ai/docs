...
| `LowCardinality(String)` | Enum-like values (`span_type`, `trace_type`, `status`, `source`) |
...
These are the logical tables exposed in the SQL Editor. The query engine rewrites them to `_v0` view functions under the hood; the schemas below reflect those view columns.
...
| `span_type` | `LowCardinality(String)` | `"LLM"` |
| `start_time` | `DateTime64(9, 'UTC')` | `"2021-01-01 00:00:00+00"` |
| `end_time` | `DateTime64(9, 'UTC')` | `"2021-01-01 00:00:00+00"` |
...
| `status` | `LowCardinality(String)` | `"success"` |
...
Status can be `"success"`, `"error"`, or `""` (empty usually indicates success).
...
| `top_span_type` | `LowCardinality(String)` | `"DEFAULT"` |
...
Status is set to `error` if any span in the trace has status `error`. Empty status means success.
...
| `timestamp` | `DateTime64(9, 'UTC')` | `"2021-01-01 00:00:00+00"` |
...
| `created_at` | `DateTime64(9, 'UTC')` | `"2021-01-01 00:00:00+00"` |
| `source` | `LowCardinality(String)` | `"MANUAL"` |

#### Source

`source` is either `"MANUAL"` (set in the Laminar UI) or `"CODE"` (attached from code).


### evaluation_datapoints (evaluation_datapoints_v0)
...
| `dataset_datapoint_created_at` | `DateTime64(9, 'UTC')` | `"1970-01-01 00:00:00+00"` |
...
### dataset_datapoints (dataset_datapoints_v0)
...
### dataset_datapoint_versions (dataset_datapoint_versions_v0)
