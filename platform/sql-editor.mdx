...
`traces` carries the signal events that fired on the trace and the clusters those events landed in, so any question of the form *which traces did X* needs no join:
...
Query [signal_events](#signal_events) directly when the row you want back is an event. See [Signal events on traces](#signal-events-on-traces) and [Clusters on traces](#clusters-on-traces) for the trace-side columns.
...
When the tuple fields are named, the shorter `alias.field` form works too, inside a lambda and after an `ARRAY JOIN`:
...
`signal_events.cluster_details` is the one exception: its fields are unnamed, so read them positionally (`c.1`, `c.2`, `c.3`) in the order the type declares.
...
#### Signal events on traces

`signal_events` holds every [signal](/signals/introduction) event that fired on the trace, so you can get from a trace to its signals without a join. Each tuple has:
...
The array is empty for traces no Signal fired on, and older traces are filled in progressively, so a trace can read empty here while its events are present in the [signal_events](#signal_events) table.

The tuple carries no timestamp or Signal name. An event happens at the end of the trace it came from, so use the trace's `end_time`, and resolve names from `signal_id`.
...
Payloads are the largest thing on the row, and reading any field of the tuple reads the payload with it. Keep the `start_time` filter, and truncate with `substring(e.payload, 1, 2000)` when selecting many rows. Counting is cheap by comparison: `empty()` and `notEmpty()` only read the array length.

#### Clusters on traces

`clusters` holds the named [clusters](/signals/clusters) this trace's signal events belong to, with names and counts already resolved. Fields match the [clusters](#clusters) table: `id`, `signal_id`, `name`, `level`, `parent_id`, `num_signal_events`, `created_at`, `updated_at`. The array is empty until the trace's events have been clustered.

Ancestors are separate elements rather than nested: a trace whose event sits in the level-1 cluster *Checkout timeouts* under the level-2 cluster *Payment failures* gets both, and you walk between them with `parent_id`. Filter on `level` to pick a granularity; `level = 1` is the finest named grouping.
...
`num_signal_events` is the size of the whole cluster, not this trace's contribution to it.
...
<Note>
If the row you want back is a trace rather than an event, read [signal events](#signal-events-on-traces) and [clusters](#clusters-on-traces) off the `traces` table instead. They answer the same questions with no join.
</Note>
...
`signal_version` tracks which revision of the Signal produced the event. It increments whenever the Signal's definition changes: its prompt, output schema, trigger, filters, or any other setting. Use it to tell events extracted under an older definition from current ones. It is `0` for events that predate versioning.
...
Unnest `leaf_clusters` to count each event exactly once. Unnesting `clusters` counts an event once per level of the hierarchy it sits in, which is what you want for a roll-up and not for a total.
...
`num_signal_events` is the cluster's own size, including its descendants. It counts cluster assignments rather than distinct events: an event that describes more than one behavior is clustered once per behavior. Unnest `traces.clusters` or `signal_events.clusters` when you need a distinct-event count.
