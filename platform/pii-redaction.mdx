...
Redaction runs at ingest, in line with the rest of trace processing. The model is Laminar's own and runs on CPU. By the time a span is searchable, in the trace view, or queryable from SQL, it has already been redacted: the original text is never persisted.
