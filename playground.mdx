...
1. In the Laminar dashboard, open **playgrounds** and click **Playground**.
2. Pick a profile and model, then start experimenting, or open directly from a span.

## Models come from LLM profiles

The model picker is built from your workspace's [LLM profiles](/platform/llm-profiles): one submenu per profile, listing the models that profile exposes. There is no built-in catalog, so a model is available here when a profile lists it and holds credentials for it. A workspace with no profiles shows **No LLM profiles configured** in place of the picker, with a link to workspace settings.
