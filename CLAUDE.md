...
- **Tags are no longer a queryable / chartable table.** Dashboards expose `traces`, `spans`, and `signal_events` as the chart-builder tables; the SQL editor reference also drops `tags`. Don't list `tags` in any "Table" enumeration on the dashboards page or example queries.
