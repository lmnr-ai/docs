...
- **Canonical evaluations layout** (under the `Evaluations` group in `docs.json`): `introduction`, `quickstart`, `concepts`, `comparing-runs`, `datasets`, `human-evaluators`, `manual-evaluation`, `self-hosted`. Do not use `using-dataset`, `configuration`, or `cookbook` slugs — those were retired in the LAM-1505 rewrite.
