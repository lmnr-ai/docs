...
- **Claude Agent SDK transcript view** — Subagents dispatched via the Agent tool (renamed from `Task` in Claude Code v2.1.63) are grouped into collapsible cards with input and final output surfaced; the orchestrator's LLM turns and tool calls render inline as a conversation instead of a span tree.
