...
- **HTTP API for manual evaluations** - The [manual evaluation](/evaluations/manual-evaluation#http-api-reference) page now documents every phase as a raw HTTP call alongside the Python and TypeScript SDKs, so you can create evaluations, stream datapoints, and write scores from any language or runtime. New `POST /v1/evals/{eval_id}` renames a run or rewrites its metadata after creation, which is what a long-running job needs when the model version or final status isn't known when the run starts.
