...
- **Manage Signals from the CLI** - New `lmnr-cli signal` commands create, update, and delete [Signals](/signals/cli) without opening the dashboard, so a Signal becomes a scriptable resource you can hand to an AI coding agent right after it debugs a failure. `create` takes a prompt and a payload schema and seeds the same default trigger the UI does; `update` is a partial patch, so changing a prompt cannot clear sampling or resume a paused Signal. `get`, `update`, and `delete` accept an id or a name, and every command supports `--json` for piping into `jq`.
