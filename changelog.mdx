...
- **AI SDK v7 metadata from `runtimeContext`** - On Vercel AI SDK v7, attach metadata, a session ID, a user ID, and tags to a call by passing them in `runtimeContext` and opting each key in with `telemetry.includeRuntimeContext`. `userId`, `sessionId`, and `tags` map to Laminar's user ID, session ID, and tags, every other key becomes trace metadata, and keys you don't opt in never leave your app. See [Attach metadata, sessions, user IDs, and tags](/integrations/vercel-ai-sdk#attach-metadata-sessions-user-ids-and-tags). Ships in `@lmnr-ai/lmnr` 0.8.49.
- **Dual PII redaction mode** - [PII redaction](/platform/pii-redaction) is now a three-way setting under **Project Settings → Security**. **On** keeps redacting at ingest for everyone, and the new **Dual** mode stores the original span text once, with detected PII marked, and redacts it per reader: workspace owners and admins see the original, members see `[REDACTED_<LABEL>]` placeholders, and project API keys and CLI sessions always see the redacted version regardless of who created them, so a coding agent can work against a project without seeing sensitive data. Spans ingested before switching to **Dual** are hidden from restricted readers rather than shown unredacted. Available on Pro and Enterprise on Laminar Cloud; self-hosted from release 0.1.790 with the [PII redactor service](/self-hosting/configuration#pii-redaction) enabled.
- **TypeSafe AI Jev instrumentation** - Laminar now auto-instruments the [TypeSafe AI SDK](/integrations/typesafe) in both TypeScript and Python, so every Jev System One call becomes an LLM span with the state, the typed questions (`Noul`, `Choice`, `Score`), the answers, and token counts. Routing, guardrail, and scoring decisions show up in the trace next to the LLM calls they gate. Python picks it up as soon as the `typesafe-sdk` package is installed; TypeScript needs `Laminar.initialize({ instrumentModules: { typesafe: TypeSafeClient } })`. Ships in `lmnr` 0.7.64 and `@lmnr-ai/lmnr` 0.8.48.
- **OpenRouter SDK instrumentation** - Laminar now auto-instruments the [OpenRouter SDK](/integrations/openrouter) in both TypeScript and Python, not just OpenRouter traffic sent through the OpenAI SDK. `chat.send`, `responses.send`, and `embeddings.generate` become LLM spans with the model, messages, response, token counts, and cost, streaming included. Python picks it up as soon as the `openrouter` package is installed; TypeScript needs `Laminar.initialize({ instrumentModules: { openrouter: OpenRouter } })` because `@openrouter/sdk` is an ES module.
- **Signal events and clusters on the traces table** - `traces` now carries the [Signal](/signals/introduction) events that fired on a trace and the named [clusters](/signals/clusters) those events landed in, names and counts already resolved. `ARRAY JOIN` either column and signal data sits next to trace cost and duration, so *which runs are in this failure cluster and what did they cost* is one query instead of two joins. On the event side, `signal_events` gains `leaf_clusters` and `cluster_details`, which resolve cluster names and levels without a join, plus `signal_version` to tell events extracted under an older Signal definition from current ones. See [SQL editor](/platform/sql-editor#table-schemas).
- **LLM profiles** - Bring your own provider credentials. An [LLM profile](/platform/llm-profiles) is a workspace-level entry holding a provider, its API keys, and the exact list of models you want to expose: OpenAI, Anthropic, Google Gemini, Groq, Mistral, AWS Bedrock, Azure AI Foundry, or any OpenAI-compatible gateway including a model you host yourself. The [playground](/playground) model picker is built from your profiles, and on self-hosted deployments each [Signal](/signals/introduction) runs on a profile and one of its models instead of the server's `LLM_PROVIDER` environment configuration. Secrets are write-only and encrypted at rest, and **Test connection** checks every model in the profile before you save.
- **CLI: LLM profiles, Signal history, and dataset CRUD** - `lmnr-cli llm-profile list / get / create / update / delete` provisions [LLM profiles](/platform/llm-profiles#manage-profiles-from-the-cli) from your terminal, with per-provider credential flags and write-only secrets. On self-hosted deployments, `signal create` and `signal update` take `--llm-profile-id` with `--model` to [run a Signal on one of those profiles](/signals/cli#run-a-signal-on-an-llm-profile), and `signal versions <signal>` prints a Signal's definition history oldest first, so you can see what the judge was asked when an old event was extracted. `dataset get`, `dataset update -n <name>`, and `dataset delete` finish off [dataset](/datasets/cli) management. Ships in `lmnr-cli` 0.5.1.

## August 2026

- **Flush span batches by payload size** - Both SDKs can now export a batch when its payload would cross a byte limit, alongside the existing span-count and scheduled-delay triggers. Turn it on with `Laminar.initialize({ flushBySize: true })` in TypeScript or `Laminar.initialize(flush_by_size=True)` in Python, and set `maxExportBatchSizeBytes` / `max_export_batch_size_bytes` if 32 MiB is not the right ceiling. Useful for agents whose spans carry whole conversation histories, where a few spans are enough to build an export the backend rejects. See [Batching](/sdk/batching).
- **Manage Signals from the CLI** - `lmnr-cli signal create / list / get / update / delete` puts the whole [Signal](/signals/introduction) definition in your terminal, so a prompt and output schema can live in your repo and ship from CI. A Signal's firing configuration is three independent flags: `--trigger` picks when it is evaluated (`root-span-finished` or `span-name`), repeatable `--filter` decides whether it runs, and `--mode` selects batch or realtime processing. `update` is a partial patch, so changing one leaves the other two alone. See [Signals CLI](/signals/cli).

## July 2026

- **Claude Code and Codex plugins** - Trace [Claude Code](/integrations/claude-code) and [OpenAI Codex CLI](/integrations/codex) sessions in Laminar with a one-command install: `npx lmnr-cli@latest plugin add claude-code` (or `codex`) logs you in, picks a project, mints a key, and installs the plugin. Each turn becomes a trace with LLM calls, tool executions, and (for Claude Code) subagents, grouped into one session per conversation.
...
- **Signal settings, backfill, and refreshed clusters** - Every Signal now has a **Settings** tab with four sections: **General** (definition and triggers), **Alerts**, **Backfill**, and **Activity**. Run a Signal across historical traces from [Backfill](/signals/quickstart#backfill-historical), watch each run's status in **Activity**, and configure a Signal's [alerts](/signals/alerts) in place, including output schema filters that fire only when an event's extracted output matches. The [clusters](/signals/clusters) view is rebuilt as a two-panel cluster list and stacked frequency chart below the events table.
- **AI SDK v7 support** - The [Vercel AI SDK](/integrations/vercel-ai-sdk) integration now covers AI SDK v7. Register Laminar once with `registerTelemetry(new LaminarAiSdkTelemetry())` and every `generateText` / `streamText` call is traced, with no per-call configuration. The v5 and v6 `experimental_telemetry` + `getTracer()` style is still documented, and the page includes a step-by-step migration guide.
...
- **Claude Agent SDK integration refresh** — Updated the [Claude Agent SDK](/integrations/claude-agent-sdk) page with end-to-end setup for TypeScript and Python, a multi-subagent example, and real transcript and tree-view screenshots.
- **OpenAI Agents SDK integration** — `Laminar.initialize()` now auto-instruments the [OpenAI Agents SDK](/integrations/openai-agents-sdk) in Python. Agent workflows, turns, tool calls, and handoffs land in Laminar as nested spans with system instructions preserved on every LLM call.
- **Pydantic AI integration** - `Laminar.initialize()` now auto-instruments [Pydantic AI](/integrations/pydantic-ai) in Python when `pydantic-ai-slim` is importable. No `Agent.instrument_all()` call or manual OTLP exporter setup is needed; raw provider instrumentors (OpenAI, Anthropic, Google GenAI, Groq, Mistral, Cohere, Bedrock) are auto-removed to avoid duplicate spans on every model call.
- **Mastra integration** — Native [Mastra](/integrations/mastra) support via `MastraExporter` wired through Mastra's `Observability` config. Captures every agent run, model step, tool call, and sub-agent. When a coordinator agent delegates to sub-agents exposed as tools, the sub-agent's full run nests under the parent tool span in one unified trace.
