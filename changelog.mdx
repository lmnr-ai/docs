...
- **Laminar Agent** - Ask an AI agent about your project's data in plain language and get answers grounded in real traces. The agent runs read-only SQL and reads trace context for you, and it runs in three places: the [MCP server](/platform/mcp) (`ask_agent` tool), the [CLI](/platform/cli#ask-the-agent) (`lmnr-cli ask`), and [Slack](/platform/integrations#use-the-agent-in-slack) (mention `@Laminar` in a connected channel). See [Laminar Agent](/platform/laminar-agent).
