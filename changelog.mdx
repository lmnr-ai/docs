...
- **Signal events and clusters on the traces table** - `traces` carries two new columns: `signal_events`, the [Signal](/signals/introduction) events that fired on the trace (`event_id`, `signal_id`, `severity`, `payload`), and `clusters`, the named [clusters](/signals/clusters) those events landed in, with `name`, `level`, `parent_id`, and counts already resolved. `ARRAY JOIN` either one and trace columns sit next to signal data, so *which runs are in this failure cluster and what did they cost* is one query instead of two joins. On the event side, `signal_events` gains `leaf_clusters` and `cluster_details` (cluster ids with names and levels resolved) and `signal_version`, which separates events extracted under an older Signal definition from current ones. `signal_events.summary` is no longer exposed in SQL. See [SQL editor](/platform/sql-editor#table-schemas).
