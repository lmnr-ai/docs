...
- **TypeSafe AI Jev instrumentation** - Laminar now auto-instruments the [TypeSafe AI Python SDK](/integrations/typesafe), so every Jev `system_one` call becomes an LLM span with the state, the typed questions (`Noul`, `Choice`, `Score`), the answers, and token counts. Routing, guardrail, and scoring decisions show up in the trace next to the LLM calls they gate. Picks up automatically when the `typesafe-sdk` package is installed; ships in `lmnr` 0.7.64.
