...
| `LMNR_DEBUG_RUN_NOTES_FILE` | Optional | Path to a file holding the pre-run note as raw markdown. The SDK reads the file and attaches its contents to the trace. Takes precedence over `LMNR_DEBUG_RUN_NOTES`. Only read when `LMNR_DEBUG` is on. |
| `LMNR_DEBUG_RUN_NOTES` | Optional | Pre-run note as inline raw markdown. Use the file variable instead when the note is more than a line. Only read when `LMNR_DEBUG` is on. |
| `LMNR_TRACE_METADATA` | Optional | JSON object attached to the trace. The `rollout.note` key holds the pre-run annotation, but the run-note variables above set it for you without hand-stringifying JSON. |
