...
Write a note **before** each run to explain what you are about to test. Write the note as raw markdown to a file and point `LMNR_DEBUG_RUN_NOTES_FILE` at it:
...
A file keeps the markdown readable and avoids hand-stringifying it into JSON. For a one-line note, set `LMNR_DEBUG_RUN_NOTES` inline instead. Both variables are only read when `LMNR_DEBUG` is on, and the file wins when both are set. The note still lands on the trace's `rollout.note` metadata key; set that key directly through `LMNR_TRACE_METADATA` if you prefer.
