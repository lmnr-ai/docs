...
2. Uses OpenAI's `gpt-5.6-luna` or Anthropic's `claude-haiku-4-5-20251001` to generate empathetic responses
...
```bash
npm install openai@latest @anthropic-ai/sdk@latest @lmnr-ai/lmnr@latest
```
...
Get your [OpenAI API key](https://platform.openai.com/api-keys) and your [Anthropic API key](https://console.anthropic.com/settings/keys) from the respective dashboards.
...
#### Passing the modules to instrument explicitly

By default Laminar patches the LLM SDKs it finds at initialization time. Next.js bundles
route code, so that automatic lookup can miss a provider you actually use. Pass the module
to `instrumentModules` and Laminar patches that exact module object:
...
Note the two different shapes: `OpenAI` is the exported class, while `anthropic` is the
whole module namespace. Other supported keys are `bedrock`, `cohere`, `langchain`,
`chromadb`, `qdrant`, and `pinecone`.

<Warning>
`instrumentModules` only takes effect if the provider packages are listed in
`serverExternalPackages` too. Otherwise Next.js bundles a separate copy of the provider
into your route, Laminar patches the unbundled one, and no spans are recorded:

```javascript next.config.ts
const nextConfig = {
  serverExternalPackages: ['@lmnr-ai/lmnr', 'openai', '@anthropic-ai/sdk'],
};
...
This is an alternative to the `Laminar.patch()` calls in step 4 below. Use one or the
other, not both: `instrumentModules` keeps all tracing setup in `instrumentation.ts`,
while `Laminar.patch()` keeps it next to the client you construct.
...
There is no Laminar-specific code here. The provider clients are already patched, so the calls below are traced as-is.
...
These files contain the LLM clients, and patch them with Laminar at the point where they're
constructed. Skip this step if you already passed the modules through `instrumentModules`
above.
...
- If you use `instrumentModules`, check that every provider package is also in `serverExternalPackages`
